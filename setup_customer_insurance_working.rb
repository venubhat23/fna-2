#!/usr/bin/env ruby
# Setup Insurance Policies for Customer - WORKING VERSION
# Usage: RAILS_ENV=development bundle exec rails runner setup_customer_insurance_working.rb
...
health_policy_2 = HealthInsurance.create!(
  ...
  policy_number: "HES#{Date.current.strftime('%Y')}#{customer.id.to_s.rjust(3, '0')}02",
...
health_policy_3 = HealthInsurance.create!(
  customer_id: customer.id,
  policy_holder: customer.display_name,
  plan_name: 'Care Health Insurance Plan',
  policy_number: "CHI#{Date.current.strftime('%Y')}#{customer.id.to_s.rjust(3, '0')}03",
...
life_policy_1 = LifeInsurance.create!(
  customer_id: customer.id,
  policy_holder: customer.display_name,
  plan_name: 'ICICI Prudential Term Plan',
  policy_number: "ICICI#{Date.current.strftime('%Y')}#{customer.id.to_s.rjust(3, '0')}01",
...
life_policy_2 = LifeInsurance.create!(
  customer_id: customer.id,
  policy_holder: customer.display_name,
  plan_name: 'Kotak Life Shield Plan',
  policy_number: "KOTAK#{Date.current.strftime('%Y')}#{customer.id.to_s.rjust(3, '0')}02",
  insurance_company_name: 'Kotak Mahindra General Insurance Company Limited',
  policy_type: 'New',
...
motor_policy_record_1 = Policy.create!(
  customer_id: customer.id,
  policy_holder: customer.display_name,
  insurance_company: 'Bajaj Allianz General Insurance Company Limited',
...
pa_policy_record = Policy.create!(
  customer_id: customer.id,
  policy_holder: customer.display_name,
  insurance_company: 'Oriental Insurance Company Limited',
