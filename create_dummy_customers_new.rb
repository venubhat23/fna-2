# Create dummy customers with all fields filled using unique mobile numbers
...
Customer.where(customer_type: 'individual').last(3).each do |customer|
  puts "#{customer.id}. #{customer.first_name} #{customer.last_name} - #{customer.email} (#{customer.mobile})"
end

puts "\n--- CORPORATE CUSTOMERS ---"
Customer.where(customer_type: 'corporate').last(3).each do |customer|
